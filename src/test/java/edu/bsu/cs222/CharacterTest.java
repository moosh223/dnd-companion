...
public class CharacterTest{
    ...
    public void testCharSTR() {
        char1.setStat(0, 12);
        int Strength = char1.getNumStats()[0];
        assert(Strength != base_STR);
        assert(Strength == char1.getNumStats()[0]);
    }
    @Test
    public void testCharDEX() {
        char1.setStat(1, 14);
        int Dexterity = char1.getNumStats()[1];
        assert(Dexterity != base_DEX);
        assert(Dexterity == char1.getNumStats()[1]);
    }
    @Test
    public void testCharCON() {
        char1.setStat(2, 8);
        int Constitution = char1.getNumStats()[2];
        assert(Constitution != base_CON);
        assert(Constitution == char1.getNumStats()[2]);
    }
    @Test
    public void testCharINT() {
        char1.setStat(3, 13);
        int Intelligence = char1.getNumStats()[3];
        assert(Intelligence != base_INT);
        assert(Intelligence == char1.getNumStats()[3]);
    }
    @Test
    public void testCharWIS() {
        char1.setStat(4, 15);
        int Wisdom = char1.getNumStats()[4];
        assert(Wisdom != base_WIS);
        assert(Wisdom == char1.getNumStats()[4]);
    }
    @Test
    public void testCharCHA() {
        char1.setStat(5, 3);
        int Charisma = char1.getNumStats()[5];
        assert(Charisma != base_CHA);
        assert(Charisma == char1.getNumStats()[5]);
    }
